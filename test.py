...
SERVICE_DESK_TICKETS_URL = f'{BASE_URL}/rest/api/2/search?jql=project%20%3D%20%22VERISV3%22%20ORDER%20BY%20created%20DESC'
SERVICE_DESK_API = f'{BASE_URL}/rest/servicedeskapi'
HEADERS = {'Accept': 'application/json', 'Content-Type': 'application/json'}
AUTH = HTTPBasicAuth("xxx.xx@text.com", "xxx")
LIMIT = 25
MAX_RESULTS = 10  # Fetch 10 tickets at a time
...
def get_issues(limit=10):
    ...
    while len(all_issues_info) < limit:
        try:
            ...
            time.sleep(RATE_LIMIT_DELAY)  # Implement rate limiting
...
def main():
    ...
    for idx, issue in enumerate(issues, start=1):
        ...
        issue['Comments'] = comments  # Store comments as a list
    ...
    with open('issues_with_comments.csv', mode='w', newline='', encoding='utf-8') as file:
        fieldnames = ['IssueID', 'IssueKey', 'IssueCreation', 'Summary', 'ReporterEmail', 'ReporterName',
                      'Description', 'Severity', 'IssueType', 'Component', 'ClientType', 'AffectedClient', 'Submitter',
                      'FixResponseMessage', 'FixRequestMessage', 'FixRequestID', 'FixSeqID', 'DealID', 'TrancheID',
                      'GUIEmailAddress', 'IssueDateAndTime', 'CurrentStatus', 'StatusTime', 'Comments']
        writer = csv.DictWriter(file, fieldnames=fieldnames)
        writer.writeheader()

        for data in all_data:
            # Format comments for CSV output
            formatted_comments = []
            for comment in data['Comments']:
                formatted_comments.append(f"ID: {comment['id']}, Author: {comment['author']}, Created: {comment['created']}, Body: {comment['body']}")
            data['Comments'] = '\n'.join(formatted_comments)  # Join comments with newline separators
